...
Defines a versioned, YAML-driven catalog of Parquet column layouts used by every signal Crashler ingests. Each schema declares its columns, the OpenTelemetry semantic-convention promotion rules that lift well-known attributes into top-level columns, and a reserved transforms block for future ingest-time mutations. The catalog is loaded at boot and validated at compile time so a malformed YAML aborts the deploy at cache-clear instead of at first request. Universal `_schema_version` and `_schema_id` columns appended by the writer let queries fan out across schema versions without ambiguity.
