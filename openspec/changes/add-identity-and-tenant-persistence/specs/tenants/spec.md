...
**Reason:** Superseded by the EasyAdmin dashboard and the upcoming user-facing UI in `add-tenant-self-service-ui`. Tenant lifecycle operations are now performed through the database-backed entities and the EasyAdmin CRUD; YAML-edit-and-redeploy remains supported via the YAML fallback source for one transition release.

**Migration:** Operators with existing YAML tenants need not act. New tenants and tokens are created in EasyAdmin. A future change will deprecate and then remove the YAML source after one release of observed dual-source operation.
