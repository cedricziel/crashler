...
- [ ] 8.1 [red] Functional test: posting to `/v1/logs` and `/v1/traces` for the same tenant in the same process produces files in the correct top-level directories (`logs/<slug>/` and `traces/<slug>/`); neither writer writes into the other's tree
- [ ] 8.2 [red] Functional test: `_schema_id` on a logs row is `logs/v1`; `_schema_id` on a trace row is `traces/v1`
...
- [ ] 11.1 `composer test` passes with zero deprecations/notices/warnings across all three suites
- [ ] 11.2 `openspec validate add-otlp-trace-ingest --strict` passes
- [ ] 11.3 CI green on main
- [ ] 11.4 `dep deploy production` (no env flag needed; additive change). Verify smoke test produces a Parquet file at `traces/<slug>/date=…/hour=…/part-…parquet` with `_schema_id = 'traces/v1'` and the full row shape
